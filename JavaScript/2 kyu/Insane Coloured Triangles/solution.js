...
const combine = (c1, c2) => c1 === c2 ? c1 : colors.find(c => c !== c1 && c !== c2);

const logBase3 = x => Math.log(x) / Math.log(3);

const triangle = (row) => {
  const len = row.length;
  if (len === 1) return row[0];

  const idealLen = Math.pow(3, Math.floor(logBase3(len - 1))) + 1;
  if (len === idealLen) return combine(row[0], row[len - 1]);

  const size = len - idealLen + 1;
  return combine(triangle(row.slice(0, size)), triangle(row.slice(len - size)));
};